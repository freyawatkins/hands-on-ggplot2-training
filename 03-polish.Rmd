...
ggplot(data, aes(x = date, y = close, color = currency)) +
  geom_line() +
  scale_x_date(
    expand = c(0, 0), ## general # remove whitespace on y axis only 
    date_breaks = "4 months", ## date-only breaks on x axis
...
ggplot(data, aes(x = date, y = close, color = currency)) +
  ...
  scale_y_continuous(
    labels = scales::dollar_format(), ## general - automatically adds $ signs 
    sec.axis = dup_axis(name = NULL), ## axis only - for duplicating y axis. but multiple! axis plots are problematic! 
...
ggplot(data, aes(x = date, y = close, color = currency)) +
  ...
  scale_color_discrete(
    type = c("#F0B90B", "#4d4d4e", "#810080", "#00aeff"),  ## color only - provide vector of hex codes
    name = "Cryptocurrency:" ## general - overwrites name of legend here rather than renaming data!!
  )
```
...
ggplot(data, aes(x = date, y = close, color = currency)) +
  geom_line() +
  scale_color_discrete(type = c("#F0B90B", "#4d4d4e","#810080", "#00aeff")) # or scale_color_manual for discrete
```
...
ggplot(data, aes(x = date, y = close, color = currency)) +
  ...
  scale_color_manual(
    ...
    labels = c("Binance", "Bitcoin", "EOS", "Litecoin"), # can also overwrite the category labels. but be careful!
    name = "Cryptocurrency:" 
  )
```
...
ggplot(data, aes(x = date, y = close, color = currency)) +
  geom_line() +
  scale_color_viridis_d() # these are tested to work well for colorblindness viridis_c (cont) & viridis_d (discrete)
```
...
ggplot(data, aes(x = date, y = close, color = open, group = currency)) +
  ...
  scale_color_viridis_c(
    option = "rocket",
    direction = -1, # check directionality of gradient coloring. i.e. this makes extreme values darkest 
    end = .9,
...
ggplot(data, aes(x = date, y = close, color = currency)) +
    ...
    scale_color_brewer(
        palette = "Set2",
        name = "Cryptocurrency:"
    ) # not the best choice here, these are developed for map fillings rather than thin lines
```
...
ggplot(sf_world) + # no need for aesthetics because the object is class sf
...
# or build own gradient
ggplot(sf_world) +
  geom_sf(aes(fill = pop_est), color = NA) +
  scale_fill_gradient(low = "grey90", high = "firebrick4")

# or build diverging color palettes
ggplot(sf_world) +
...
# don't really see diverging data because midpoint is centred around 0

# or specify custom midpoint
ggplot(sf_world) +
  geom_sf(aes(fill = as.numeric(factor(income_grp))), color = NA) +
  scale_fill_gradient2(low = "darkcyan", high = "firebrick4", mid = "grey90", name = "Income group:",
                       midpoint = 3)
...
```{r guide-default}
g +
  guides(fill = guide_colorbar()) #continuous legend
```

```{r guide-legend }
g +
  guides(fill = guide_legend()) # single groups in legend
```
...
g +
  guides(fill = guide_legend(reverse = TRUE, keyheight = unit(2, "lines"))) # adapt size of legend boxes
```

```{r guide-colorbar-height-width }
g +
  guides(fill = guide_colorbar(barheight = unit(12, "lines"),
                               barwidth = unit(1, "lines"))) # add lines separating each on continuous legend
```
...
```{r}
data %>% ggplot(aes(close, open, color = high)) +
...
ggplot(data, aes(x=date, y=close, color=high)) +
  ...
  coord_polar() +
  ggtitle("I went to Cedric's ggplot2 course and\nall I got is this ridiculous chart") +
