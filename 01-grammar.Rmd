...
```{r Exercise_1}
chicago <- read_csv("https://raw.githubusercontent.com/z3tt/ggplot-courses/master/data/chicago-nmmaps.csv")
glimpse(chicago)
unique(chicago$year)
```
...
ggplot(chicago, aes(date, temp, color = season)) +
  geom_point()

ggplot(chicago, aes(date, temp, color = factor(year))) +
  geom_point() # without factor(year), ggplot treats year as continuous gradient, not categorical, for the colour legend

ggplot(chicago, aes(date, temp, color = season, group = year)) +
  geom_line() # oh no, without group = year, ggplot connects seasons by line between years
...
ggsave(filename = "my_ggplot.pdf", 
       width = 10, height = 7, 
       device = cairo_pdf) # for saving pdf (or .svg) which are vector graphics, dont get pixelated

ggsave(filename = "my_ggplot.png", 
       width = 10, height = 7, 
       dpi = 700) # for saving png. png is a raster graphic tho which becomes pixelated
```
...
# json file is missing, so below is commented out
#```{r data-rio-json}
