...
ggplot(data, aes(date, close, color = currency)) +
  geom_line() + 
  xlab(NULL) + # we can use xlab() but this method is more laborious than using labs() for all
...
# ok but **s are showing up on the graph instead of bolding - this is why we need ggtext theme call, see next chunk
```
...
ggplot(data, aes(date, close, color = currency)) +
  ...
  labs(title = "Performance of the <b style='font-size:34pt;'>Top 4 Cryptocurrencies ", 
       caption = "<span style='color:firebrick;'>Data:</span> <i>CoinMarketCap.com</i>") + # can use html too
  theme(plot.title = element_markdown(), plot.caption = element_markdown())
```
...
ggplot(outliers, aes(close, open)) + 
  ...
  geom_text(
    ...
    nudge_x = 5,
    nudge_y = -5
  ) # moving them all the same way though is not very helpful
```
...
`magick::image_read()` allows us to directly store images from the web in `R`:
...
ggplot(data, aes(date, close, color = currency)) +
  annotation_custom(
    grid::rasterGrob(
        image = img,
        x = .5,
        y = -.25, # now we can plot the image below the plot
        width = .9 
    ) 
  ) +
  geom_line(size = .8) +
  coord_cartesian(clip = "off") +
  theme(plot.margin = margin(12, 12, 130, 12)) # create negative space (top, right, bottom, left TRBL)
```
...
my_plot <- ggplot(data, aes(close, open)) + 
  geom_point()
add_logo(my_plot)
```
