...
e.g if we don't want to stack them, use the position = dodge argument to (here) put them on top of each other.
...
ggplot(data, aes(date, close, color = currency)) +
  geom_area(aes(fill = currency), alpha = .5, position = "fill") +
  coord_cartesian(expand = FALSE) # removes whitespace on x and y axes
```
...
ggplot(data, aes(close, currency)) +
  geom_boxplot() # calculates median & IQR to show 50% in box by default, plus whiskers with outliers etc
```
...
```{r boxplot-dots}
ggplot(data, aes(close, currency)) +
  geom_boxplot(outlier.alpha = 0, color = "grey60", size = 1) + #makes boxplot transparent!
  geom_point(alpha = .2)
```
...
ggplot(data, aes(close, currency)) +
  ...
  geom_point(alpha = .2, position = position_jitter(height = .2)) # moves points nicely
```
...
Means you don't have to create your own summary tables with means + SE first.
```{r stats-summary}
...
```{r Exercise-1}
passwords <- read_csv("https://raw.githubusercontent.com/Z3tt/ggplot-courses/master/data/passwords.csv")
glimpse(passwords)
...
ggplot(pw_count, aes(fct_reorder(category, -n), n)) +
  geom_col() +
  geom_text(aes(label = n), vjust = -.6, size = 6) + # add text labels with geom_text
  scale_y_continuous(limits = c(NA, 210), expand = c(0, 0)) 

# ...or with stat_count instead of geom_text() without requiring summary dataset first
ggplot(passwords, aes(category)) +
  stat_count() +
  stat_count(geom = "text", aes(label = ..count..), vjust = -.6, size = 6) +
  scale_y_continuous(limits = c(NA, 210), expand = c(0, 0))

# or indeed with stat_summary(), but this is the most complex and least common way to do this
...
# having server issues, all chunks from here to the end are therefore include = F bc data won't download
...
```{r first-map}
ggplot(sf_world) +
  geom_sf() # no need for X and Y because obvious plotting long and lat.
...
```{r sf-polygon-point, include = F}
ggplot(sf_world) +
  geom_sf() #+
 # geom_sf(data = sf_airports) # need to specify data as first argument, to plot airports on top.
```

```{r first-map-colors, include= F}
ggplot(sf_world) +
  geom_sf(color = "forestgreen", alpha = .4) #+ # color outline
  #geom_sf(data = sf_airports, color = "purple", alpha = .4) # plots purple points of airpoint data
```

Generate simple features objects e.g. to highlight a location:

```{r data-locator}
sf_bln <- sf::st_sfc(sf::st_point(c(13.4050, 52.5200)), crs = sf::st_crs(sf_world)) # longlat data, we can use the geometry of the world to define it. 
...
ggplot(sf_world) +
  geom_sf(color = NA) +
  geom_sf(data = sf_bln, shape = 21, color = "firebrick", fill = NA, size = 5, stroke = 2) # one dot is the data
```
...
ggplot(sf_world) +
  geom_sf(aes(fill = economy)) # we can map other values to the fill 
 
```

```{r first-map-color}
ggplot(sf_world) +
  geom_sf(aes(fill = economy), color = "white", size = .2) # add white lines
```
...
ggplot(sf_world) +
  ...
  coord_sf(crs = "+proj=moll") # change projection on the fly
```

```{r first-map-proj-custom1}
ggplot(sf_world) +
  geom_sf(aes(fill = economy), color = "white", size = .2) +
  coord_sf(crs = "+proj=laea +x_0=0 +y_0=0 +lon_0=0 +lat_0=0") # globe-like projection
```

```{r first-map-proj-custom2}
ggplot(sf_world) +
  geom_sf(aes(fill = economy), color = "white", size = .2) +
  coord_sf(crs = "+proj=laea +x_0=0 +y_0=0 +lon_0=-30 +lat_0=-25") # turn it around
```

```{r first-map-proj-custom3}
ggplot(sf_world) +
  geom_sf(aes(fill = economy), color = "white", size = .2) +
  coord_sf(crs = "+proj=laea +x_0=0 +y_0=0 +lon_0=-70 +lat_0=0") # failed visualisation here. 
```
...
# requires {rnaturalearthhires}, server issue installing from source

class(sf_germany)
names(sf_germany)

ggplot(sf_germany) +
  geom_sf(fill = "#CBE7BE") # sf_germany_hd would be much more accurate

ggplot(sf_germany) +
  geom_sf(fill = "#CBE7BE") +
  geom_sf(data = sf_bln, size = 5) + # add berlin point
  coord_sf(crs = "+proj=moll") # change projection
